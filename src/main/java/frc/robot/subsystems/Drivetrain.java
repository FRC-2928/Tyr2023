...
// OLD Tyr wanted this   import edu.wpi.first.wpilibj.RobotDrive;
...
public class Drivetrain extends SubsystemBase {
    ...
    private final WPI_TalonSRX m_leftFollower = new WPI_TalonSRX(backLeftDrive);  // m_leftFollower ??
  ...
  /// TODO Typ Gyro  private final BasePigeonSimCollection m_pigeonSim = m_pigeon.getSimCollection();
    ...
    public Drivetrain(Supplier<Transmission.GearState> gearStateSupplier) {
        ...
        Pose2d initialPose = new Pose2d(0, 1.5, gyro.getRotation2d());    
    ...
    public void configmotors() {
        ...
        for(TalonSRX srx : new TalonSRX[] {m_leftLeader, m_leftFollower, m_rightLeader, m_rightFollower}){
        ...
        // OR LIKE THIS for SRX?????? see Vikinmg robot 2018
        // m_leftFollower.set(ControlMode.Follower, frontLeftDrive);
    ...
    public void setWheelPIDF() {
        ...
        for(TalonSRX srx : new TalonSRX[] {m_leftLeader, m_rightLeader}){
            
            srx.config_kP(0, DrivetrainConstants.kGainsProfiled.kP, 0);
            srx.config_kI(0, DrivetrainConstants.kGainsProfiled.kI, 0);
            srx.config_kD(0, DrivetrainConstants.kGainsProfiled.kD, 0);
            srx.config_kF(0, DrivetrainConstants.kGainsProfiled.kF, 0);
...
//        outputPid = levelShooterPid.calculate(m_lifter.anglePos(), (m_pigeon.getPitch()+Constants.DrivetrainConstants.absPosition));
    ...
    public Rotation2d getRotation(){
        m_yaw = gyro.getAngle(); //   m_yaw = m_pigeon.getYaw();
        if (RobotBase.isReal()) {
